...
func (p *Parser) Parse(ctx context.Context, params models.ParseParams) (*models.Product, error) {
  log.
    ...
    Debug("oktyabr product parsing started")
  ...
  for _, parsedOffer := range parsed.Offers {
    ...
    if !ok || !matchSize(sizeString, paramsSizesSet) {
      log.
        ...
        Debug("oktyabr parsed size not match to params: size will be skipped")
    ...
    if !ok {
      log.
        ...
        Debug("oktyabr parsed size not match to parsed stocks: size will be skipped")
    ...
    if err != nil {
      log.
        ...
        Errorf("oktyabr make product option error: %v", err)
  ...
  log.
    ...
    Debug("oktyabr product parsed successfully")
